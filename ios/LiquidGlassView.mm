...
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  ...
  // We need to update interactive before other properties,
  // since it needs to reset the effect when it changes
  // So a tint or style change in the same update can still animate
  if (oldViewProps.interactive != newViewProps.interactive) {
    _view.interactive = newViewProps.interactive;
    [_view resetInteractiveEffect];
  }
